...
// Type: PointBlank.Battle.Network.BattleHandler
  ...
  public class BattleHandler
  {
    ...
    public void BeginReceive(byte[] Buffer, DateTime Date)
    {
      ...
      if (packetModel.Length > packetModel.Data.Length)
      {
        ...
        Logger.warning("Packet with invalid size canceled. [Length: " + packetModel.Length.ToString() + " DataLength: " + packetModel.Data.Length.ToString() + "]");
      }
      else
      {
        this.getDecryptedData(packetModel);
        if (BattleConfig.isTestMode && packetModel.Unk > 0)
          Logger.warning("Unk: " + packetModel.Unk.ToString());
      ...
      try
      {
        switch (Packet.Opcode)
        {
          ...
          case 4:
            ...
            if (player1 != null && player1.AccountIdIsValid(Packet.AccountId))
            {
              ...
              if (flag1)
                ...
                Logger.warning("DedicationSlotId: " + num3.ToString() + " Slot: " + Packet.Slot.ToString() + "  Opcode: " + Packet.Opcode.ToString());
            ...
            break;
          case 65:
            short num4 = receivePacket.readH();
            string str1 = num4.ToString();
            num4 = receivePacket.readH();
            ...
            if (player3 != null)
            {
              ...
              if (BattleConfig.isTestMode)
                Logger.warning("Player Connected. [" + player3.Client.Address?.ToString() + ":" + player3.Client.Port.ToString() + "]");
            }
            break;
          case 67:
            ...
            if (room2.RemovePlayer(Packet.Slot, this.RemoteEP) && BattleConfig.isTestMode)
              Logger.warning("Player Disconnected. [" + this.RemoteEP.Address?.ToString() + ":" + this.RemoteEP.Port.ToString() + "]");
          ...
          case 132:
            ...
            int num5 = (int) receivePacket.readC();
            num2 = receivePacket.readUD();
            ...
            {
              room4.BotMode = true;
              Player player6 = num5 != (int) byte.MaxValue ? room4.getPlayer(num5, false) : (Player) null;
              byte[] data2 = player6 == null ? PROTOCOL_BOTS_ACTION.getCode(noEndData, player5.Date, Packet.Round, Packet.Slot) : PROTOCOL_BOTS_ACTION.getCode(noEndData, player6.Date, Packet.Round, num5);
              for (int index = 0; index < 16; ++index)
              {
                Player player7 = room4.Players[index];
                if (player7.Client != null && player7.AccountIdIsValid() && index != Packet.Slot)
                  this.Send(data2, player7.Client);
              }
            }
            break;
          default:
            Logger.warning("Invalid Packet: " + Packet.Opcode.ToString());
      ...
      out byte[] EventsData)
    {
      ...
      using (SendPacket sendPacket = new SendPacket())
      {
        ...
        if (Action.Flag.HasFlag((System.Enum) UDP_GAME_EVENTS.ActionState))
        {
          ...
          if (!Room.BotMode && player != null)
          {
            int num2 = 0;
            byte num3 = 0;
            if (info.Action.HasFlag((System.Enum) ACTION_STATE.WEAPONSYNC))
            {
              if (info.Flag.HasFlag((System.Enum) WEAPON_SYNC_TYPE.PRIMARY))
                num2 = player.Primary;
              if (info.Flag.HasFlag((System.Enum) WEAPON_SYNC_TYPE.SECONDARY))
                num2 = player.Secondary;
              if (info.Flag.HasFlag((System.Enum) WEAPON_SYNC_TYPE.MELEE))
                num2 = player.Knife;
              if (info.Flag.HasFlag((System.Enum) WEAPON_SYNC_TYPE.GRENADE))
                num2 = player.Grenade;
              if (info.Flag.HasFlag((System.Enum) WEAPON_SYNC_TYPE.SPECIAL))
                num2 = player.Special;
              if (info.Flag.HasFlag((System.Enum) WEAPON_SYNC_TYPE.MISSION) && Room.RoomType == ROOM_STATE_TYPE.Bomb)
                num2 = 5009001;
              if (info.Flag.HasFlag((System.Enum) WEAPON_SYNC_TYPE.DUAL))
              {
                num3 = (byte) 16;
                num2 = player.Primary;
              }
            }
            objectHitInfoList.Add(new ObjectHitInfo(5)
            {
              ObjId = player.Slot,
              WeaponId = num2,
              Extensions = num3
        ...
        {
          num1 += 64;
          WeaponSyncInfo info = WeaponSync.ReadInfo(Action, receivePacket, false);
        ...
        {
          num1 += 512;
          List<SuicideInfo> hits = Suicide.ReadInfo(receivePacket, false);
          int weaponId = 0;
          if (player != null)
          {
            ...
            for (int index = 0; index < hits.Count; ++index)
            {
              ...
              {
                int num4 = (int) (suicideInfo.HitInfo >> 20);
                CHARA_DEATH charaDeath = (CHARA_DEATH) ((int) suicideInfo.HitInfo & 15);
                int num5 = (int) (suicideInfo.HitInfo >> 11) & 511;
                ...
                if (assistModel.Killer != assistModel.Victim)
        ...
        {
          num1 += 1024;
          MissionDataInfo info = MissionData.ReadInfo(Action, receivePacket, false, Time);
          if (Room.Map != null && player != null && !player.Dead && (double) info.PlantTime > 0.0 && !info.BombEnum.HasFlag((System.Enum) BOMB_FLAG.STOP))
          {
            ...
            if (bomb != null)
            {
              ...
              double num6 = (double) Vector3.Distance((Vector3) player.Position, vector3);
              if ((bomb.EveryWhere || num6 <= 2.0) && (player.Team == 1 & flag || player.Team == 0 && !flag))
              {
                ...
                if (((double) Time >= (double) info.PlantTime + (double) num7 || totalSeconds >= (double) num7) && (!Room.HasC4 && info.BombEnum.HasFlag((System.Enum) BOMB_FLAG.START) || Room.HasC4 & flag))
                {
        ...
        {
          num1 += 131072;
          DropWeaponInfo info = DropWeapon.ReadInfo(receivePacket, false);
          if (Room != null && !Room.BotMode)
          {
            ...
            if (player != null)
            {
              int idStatics = AllUtils.getIdStatics(info.WeaponId, 1);
              if (idStatics == 1)
                player.Primary = 0;
              if (idStatics == 2)
                player.Secondary = 0;
          ...
          DropWeapon.WriteInfo(sendPacket, info, Room != null ? Room.DropCounter : 0);
        }
        if (Action.Flag.HasFlag((System.Enum) UDP_GAME_EVENTS.GetWeaponForClient))
        {
          num1 += 262144;
          WeaponClient info = GetWeaponForClient.ReadInfo(Action, receivePacket, false);
          if (!Room.BotMode && player != null)
          {
            int idStatics = AllUtils.getIdStatics(info.WeaponId, 1);
            if (idStatics == 1)
              player.Primary = info.WeaponId;
            if (idStatics == 2)
              player.Secondary = info.WeaponId;
          }
          GetWeaponForClient.WriteInfo(sendPacket, info);
        }
        ...
        {
          num1 += 2097152;
          List<HitDataInfo> Hits = HitData.ReadInfo(receivePacket, false);
          ...
          if (player != null)
          {
            for (int index = 0; index < Hits.Count; ++index)
            {
              ...
              {
                double num9 = (double) Vector3.Distance((Vector3) Hit.StartBullet, (Vector3) Hit.EndBullet);
                if (Hit.WeaponClass == CLASS_TYPE.Knife && (MeleeExceptionsXml.Contains(AllUtils.getIdStatics(Hit.WeaponId, 3)) || num9 < 3.0) || Hit.WeaponClass != CLASS_TYPE.Knife)
                {
                  int damage = (int) AllUtils.getHitDamageNormal(Hit.HitIndex);
                  int hitWho = AllUtils.getHitWho(Hit.HitIndex);
                  int hitPart = AllUtils.getHitPart(Hit.HitIndex);
                  ...
                  switch (hitType)
                  {
                    case OBJECT_TYPE.User:
                      ...
                      if (Room.getPlayer(hitWho, out Player) && player.RespawnLogicIsValid() && !player.Dead && !Player.Dead && !Player.Immortal)
                      {
                        ...
                        if (assistModel.Killer != assistModel.Victim)
                          DamageManager.Assists.Add(assistModel);
                        DamageManager.SimpleDeath(deaths, objectHitInfoList, player, Player, damage, num8, hitPart, deathType);
                        break;
                      }
                      this.RemoveHit((IList) Hits, index--);
                      break;
                    ...
                    case OBJECT_TYPE.Object:
                      ...
                      if (objM != null && objM.Destroyable)
                      {
                        if (objectInfo.Life > 0)
                        {
                          ...
                          {
                            objectInfo.Life = 0;
                            DamageManager.BoomDeath(Room, player, num8, deaths, objectHitInfoList, Hit.BoomPlayers);
                          }
                      ...
                      {
                        Logger.warning("Fire Obj: " + hitWho.ToString() + " Map: " + Room.MapId.ToString() + " Invalid Object.");
                        player.LogPlayerPos(Hit.EndBullet);
                        break;
                      }
                      break;
                    default:
                      Logger.warning("HitType: (" + hitType.ToString() + "/" + ((int) hitType).ToString() + ") Slot: " + Action.Slot.ToString());
                      Logger.warning("BoomPlayers: " + Hit.BoomInfo.ToString() + " " + Hit.BoomPlayers.Count.ToString());
                      break;
                  }
                }
                else
            ...
            if (deaths.Count > 0)
              BattleSync.SendDeathSync(Room, player, (int) byte.MaxValue, num8, deaths);
          }
          else
        ...
        {
          num1 += 4194304;
          List<GrenadeHitInfo> hits = GrenadeHit.ReadInfo(receivePacket, false);
          List<DeathServerData> deathServerDataList = new List<DeathServerData>();
          int weaponId = 0;
          if (player != null)
          {
              ...
              switch (hitType)
              {
                case OBJECT_TYPE.User:
                  ...
                  if (damage > 0 && Room.getPlayer(hitWho, out Player) && player.RespawnLogicIsValid() && !Player.Dead && !Player.Immortal)
                  {
                    ...
                    else if (grenadeHitInfo.DeathType == (byte) 2 && CLASS_TYPE.Dino != grenadeHitInfo.WeaponClass && num12 % 2 == 0)
                    {
                ...
                case OBJECT_TYPE.Object:
                  ...
                  if (BattleConfig.sendFailMsg && objM == null)
                  {
                    Logger.warning("Boom Obj: " + hitWho.ToString() + " Map: " + Room.MapId.ToString() + " Invalid Object.");
                    player.LogPlayerPos(grenadeHitInfo.HitPos);
                    break;
                  }
                  break;
                default:
                  Logger.warning("Grenade Boom, HitType: (" + hitType.ToString() + "/" + ((int) hitType).ToString() + ")");
                  break;
              }
            }
            if (deathServerDataList.Count > 0)
        ...
        if ((long) num1 != (long) Action.Flag)
          Logger.warning("[" + ((uint) Action.Flag).ToString() + " | " + ((long) Action.Flag - (long) num1).ToString());
            ...
            if (!exception)
            {
              ...
              if (Action.Length != ushort.MaxValue)
              {
                ...
                else if (Action.SubHead == UDP_SUB_HEAD.USER || Action.SubHead == UDP_SUB_HEAD.STAGEINFO_CHARA)
                {
                  ...
                  Logger.warning("[SubHead: '" + Action.SubHead.ToString() + "' or '" + ((int) Action.SubHead).ToString() + "']");
              }
              else
                break;
            }
    ...
    private void Send(byte[] data, IPEndPoint ip) => this.Client.Send(data, data.Length, ip);
  }
}
