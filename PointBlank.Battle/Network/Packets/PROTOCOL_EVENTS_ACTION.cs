...
// Type: PointBlank.Battle.Network.Packets.PROTOCOL_EVENTS_ACTION
...
namespace PointBlank.Battle.Network.Packets
{
  ...
  {
    public static byte[] getCode(byte[] actions, DateTime date, int round, int slot) => PROTOCOL_EVENTS_ACTION.BaseGetCode(AllUtils.Encrypt(actions, (13 + actions.Length) % 6 + 1), date, round, slot);
      ...
      using (SendPacket sendPacket = new SendPacket())
      {
        for (int index = 0; index < objs.Count; ++index)
        {
          ...
          else if (objectHitInfo.Type == 5)
          {
            ...
            sendPacket.writeC(objectHitInfo.Extensions);
            sendPacket.writeD(objectHitInfo.WeaponId);
          }
        }
        return sendPacket.mstream.ToArray();
