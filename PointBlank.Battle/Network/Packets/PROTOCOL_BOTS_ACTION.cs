...
// Type: PointBlank.Battle.Network.Packets.PROTOCOL_BOTS_ACTION
          ...
          try
          {
            ...
            if (!exception)
            {
              ...
              if (actionModel.Length != ushort.MaxValue)
              {
                ...
                {
                  Logger.warning("[New User Packet Type: '" + actionModel.SubHead.ToString() + "' or '" + ((int) actionModel.SubHead).ToString() + "']: " + BitConverter.ToString(data));
