...
// Type: PointBlank.Battle.Network.Actions.Damage.DamageManager
...
namespace PointBlank.Battle.Network.Actions.Damage
{
  public class DamageManager
  {
      ...
      CHARA_DEATH DeathType)
    {
            ...
            {
                AssistModel assistModel1 = DamageManager.Assists.Find((Predicate<AssistModel>)(x => x.Victim == Player.Slot));
                Player.Life = 0;
                Player.Dead = true;
                Deaths.Add(new DeathServerData()
                {
                    Player = Player,
                    DeathType = DeathType,
                    Assist = assistModel1 == null ? Killer.Slot : (assistModel1.IsAssist ? assistModel1.Killer : Killer.Slot)
                });
                DamageManager.Assists.Remove(assistModel1);
                foreach (AssistModel assistModel2 in DamageManager.Assists.FindAll((Predicate<AssistModel>)(x => x.Victim == Player.Slot)))
                    DamageManager.Assists.Remove(assistModel2);
            }
        }
        ...
        if (room.getPlayer(boomPlayer, out Player) && !Player.Dead)
        {
          ...
          objs.Add(new ObjectHitInfo(2)
          {
            ...
            WeaponId = weaponId
                ...
                foreach (AssistModel assistModel in DamageManager.Assists.FindAll((Predicate<AssistModel>)(x => x.RoomId == room.RoomId)))
