...
// Type: PointBlank.Auth.Data.Managers.ClanManager
    ...
    public static PointBlank.Core.Models.Account.Clan.Clan getClanDB(object valor, int type)
    {
      try
      {
        ...
        using (NpgsqlConnection npgsqlConnection = SqlConnection.getInstance().conn())
        {
          ...
          command.CommandText = "SELECT * FROM clans WHERE " + str + "=@valor";
      ...
      try
      {
        using (NpgsqlConnection npgsqlConnection = SqlConnection.getInstance().conn())
        {
          ...
          while (npgsqlDataReader.Read())
          {
            ...
            if (int64 != exception)
            {
              PointBlank.Auth.Data.Model.Account account = new PointBlank.Auth.Data.Model.Account()
              {
                player_id = int64,
