...
// Type: PointBlank.Game.Network.ClientPacket.PROTOCOL_AUTH_SHOP_DELETE_ITEM_REQ
...
namespace PointBlank.Game.Network.ClientPacket
{
  public class PROTOCOL_AUTH_SHOP_DELETE_ITEM_REQ : ReceivePacket
  {
    ...
    public PROTOCOL_AUTH_SHOP_DELETE_ITEM_REQ(GameClient client, byte[] data) => this.makeme(client, data);
    ...
    public override void run()
    {
      try
      {
        ...
        else if (ComDiv.getIdStatics(itemsModel._id, 1) == 16)
        {
          if (bonus == null)
          {
            this._client.SendPacket((SendPacket) new PROTOCOL_AUTH_SHOP_DELETE_ITEM_ACK(2147483648U));
            return;
          }
          if (!bonus.RemoveBonuses(itemsModel._id))
          {
            ...
            else if (itemsModel._id == 1600010)
            {
              if (bonus.fakeNick.Length == 0)
                this.erro = 2147483648U;
              else if (ComDiv.updateDB("players", "player_name", (object) bonus.fakeNick, "player_id", (object) player.player_id) && ComDiv.updateDB("player_bonus", "fakenick", (object) "", "player_id", (object) player.player_id))
              {
            ...
            {
              if (ComDiv.updateDB("players", "name_color", (object) 0, "player_id", (object) player.player_id))
              {
                player.name_color = 0;
