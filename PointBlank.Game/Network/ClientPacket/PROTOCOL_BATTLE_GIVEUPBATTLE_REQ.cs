...
// Type: PointBlank.Game.Network.ClientPacket.PROTOCOL_BATTLE_GIVEUPBATTLE_REQ
...
namespace PointBlank.Game.Network.ClientPacket
{
  public class PROTOCOL_BATTLE_GIVEUPBATTLE_REQ : ReceivePacket
  {
    ...
    public PROTOCOL_BATTLE_GIVEUPBATTLE_REQ(GameClient client, byte[] data) => this.makeme(client, data);
    ...
    private void FreepassEffect(Account p, PointBlank.Core.Models.Room.Slot slot, PointBlank.Game.Data.Model.Room room, bool isBotMode)
    {
      ...
      else
      {
        ...
        {
          int num7 = slot.allKills != 0 || slot.allDeaths != 0 ? (int) slot.inBattleTime(DateTime.Now) : 0;
          if (room.room_type == RoomType.Bomb || room.room_type == RoomType.FreeForAll || room.room_type == RoomType.Convoy)
          {
            num4 = (int) ((double) slot.Score + (double) num7 / 2.5 + (double) slot.allDeaths * 2.2 + (double) (slot.objects * 20));
            num3 = (int) ((double) slot.Score + (double) num7 / 3.0 + (double) slot.allDeaths * 2.2 + (double) (slot.objects * 20));
          }
          else
          {
            num4 = (int) ((double) slot.Score + (double) num7 / 2.5 + (double) slot.allDeaths * 1.8 + (double) (slot.objects * 20));
            num3 = (int) ((double) slot.Score + (double) num7 / 3.0 + (double) slot.allDeaths * 1.8 + (double) (slot.objects * 20));
          }
        }
      ...
      ComDiv.updateDB("players", "player_id", (object) p.player_id, dbQuery.GetTables(), dbQuery.GetValues());
    }
