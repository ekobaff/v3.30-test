...
// Type: PointBlank.Game.Network.ClientPacket.PROTOCOL_CLAN_WAR_JOIN_TEAM_REQ
...
namespace PointBlank.Game.Network.ClientPacket
{
  public class PROTOCOL_CLAN_WAR_JOIN_TEAM_REQ : ReceivePacket
  {
    ...
    public PROTOCOL_CLAN_WAR_JOIN_TEAM_REQ(GameClient client, byte[] data) => this.makeme(client, data);
    ...
    public override void run()
    {
      try
      {
        ...
        {
          this._client.SendPacket((SendPacket) new PROTOCOL_CLAN_WAR_JOIN_TEAM_ACK(2147483648U));
        }
        else
        {
          ...
          if (channel != null)
          {
            if (player.clanId == 0)
            {
              this._client.SendPacket((SendPacket) new PROTOCOL_CLAN_WAR_JOIN_TEAM_ACK(2147487835U));
            }
            else
            {
