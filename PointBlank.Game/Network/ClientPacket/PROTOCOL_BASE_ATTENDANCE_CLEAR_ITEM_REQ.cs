...
// Type: PointBlank.Game.Network.ClientPacket.PROTOCOL_BASE_ATTENDANCE_CLEAR_ITEM_REQ
...
namespace PointBlank.Game.Network.ClientPacket
{
  public class PROTOCOL_BASE_ATTENDANCE_CLEAR_ITEM_REQ : ReceivePacket
  {
    ...
    public PROTOCOL_BASE_ATTENDANCE_CLEAR_ITEM_REQ(GameClient client, byte[] data) => this.makeme(client, data);
      ...
      try
      {
        ...
        else if (player._event != null)
        {
          ...
          else
          {
            ...
            else if (runningEvent.EventIsEnabled())
            {
              VisitItem reward = runningEvent.getReward(player._event.LastVisitSequence2, this.type);
              if (reward != null)
              {
                ...
                if (good != null)
                {
                  ...
                  this._client.SendPacket((SendPacket) new PROTOCOL_INVENTORY_GET_INFO_ACK(0, player, new ItemsModel(good._item._id, good._item._category, good._item._name, good._item._equip, reward.count)));
                  this._client.SendPacket((SendPacket) new PROTOCOL_SERVER_MESSAGE_ANNOUNCE_ACK(Translation.GetLabel("AttendanceCheck")));
                }
                else
