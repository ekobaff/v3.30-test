...
// Type: PointBlank.Game.Network.ClientPacket.PROTOCOL_CS_ACCEPT_REQUEST_REQ
...
namespace PointBlank.Game.Network.ClientPacket
{
  public class PROTOCOL_CS_ACCEPT_REQUEST_REQ : ReceivePacket
  {
    ...
    public PROTOCOL_CS_ACCEPT_REQUEST_REQ(GameClient client, byte[] data) => this.makeme(client, data);
        ...
        else
        {
          ...
          for (int index = 0; index < num; ++index)
          {
            ...
            if (account != null && clanPlayers.Count < clan.maxPlayers && account.clanId == 0 && PlayerManager.getRequestClanId(account.player_id) > 0)
            {
              ...
              ComDiv.updateDB("players", "player_id", (object) account.player_id, new string[3]
              {
                "clanaccess",
                "clan_id",
                "clandate"
