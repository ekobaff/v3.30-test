...
// Type: PointBlank.Game.Network.ClientPacket.PROTOCOL_AUTH_SHOP_ITEM_AUTH_REQ
...
namespace PointBlank.Game.Network.ClientPacket
{
  public class PROTOCOL_AUTH_SHOP_ITEM_AUTH_REQ : ReceivePacket
  {
    ...
    public PROTOCOL_AUTH_SHOP_ITEM_AUTH_REQ(GameClient client, byte[] data) => this.makeme(client, data);

    public override void read() => this.objId = (long) this.readD();
    ...
    public override void run()
    {
      ...
      try
      {
        ...
        if (itemsModel1 != null)
        {
          ...
          {
            if (ComDiv.updateDB("players", "escapes", (object) 0, "player_id", (object) player.player_id))
            {
          ...
          else if (this.itemId == 1800055)
          {
            ...
            if (clan._id > 0 && clan.owner_id == this._client.player_id)
            {
              if (clan.maxPlayers + 50 <= 250 && ComDiv.updateDB("clans", "max_players", (object) (clan.maxPlayers + 50), "clan_id", (object) player.clanId))
              {
                clan.maxPlayers += 50;
          ...
          else if (this.itemId == 1800056)
          {
            ...
            {
              if (ComDiv.updateDB("clans", "pontos", (object) 1000f, "clan_id", (object) player.clanId))
              {
                clan._pontos = 1000f;
          ...
          else if (this.itemId > 1800113 && this.itemId < 1800119)
          {
            ...
            if (ComDiv.updateDB("players", "gp", (object) (player._gp + increase), "player_id", (object) player.player_id))
            {
              player._gp += increase;
          ...
          else if (itemsModel1._category == 3 && RandomBoxXml.ContainsBox(this.itemId))
          {
            ...
            if (box != null)
            {
              ...
              if (rewardList.Count > 0)
              {
                ...
                if (Rewards.Count > 0)
                {
                  for (int index3 = 0; index3 < Rewards.Count; ++index3)
                  {
                    ItemsModel itemsModel2 = Rewards[index3];
...
label_77:
      ...
      {
        Logger.error("Obj: " + this.objId.ToString() + " ItemId: " + this.itemId.ToString() + " Count: " + this.oldCOUNT.ToString() + " PlayerId: " + this._client.player_id.ToString() + " Name: '" + this._client._player.player_name + "'\r\n" + ex.ToString());
    ...
    private void cupomIncreaseDays(PointBlank.Game.Data.Model.Account p, string originalName)
    {
      ...
      else
      {
        ...
        if (itemsModel1 == null)
        {
          ...
          int int64 = Convert.ToInt32(dateTime.ToString("yyMMddHHmm"));
          ItemsModel itemsModel2 = new ItemsModel(id, 3, name, 2, int64);
