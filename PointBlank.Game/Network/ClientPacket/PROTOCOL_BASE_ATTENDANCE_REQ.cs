...
// Type: PointBlank.Game.Network.ClientPacket.PROTOCOL_BASE_ATTENDANCE_REQ
...
namespace PointBlank.Game.Network.ClientPacket
{
  public class PROTOCOL_BASE_ATTENDANCE_REQ : ReceivePacket
  {
    ...
    public PROTOCOL_BASE_ATTENDANCE_REQ(GameClient client, byte[] data) => this.makeme(client, data);
    ...
    public override void run()
    {
      try
      {
        ...
        else if (player._event != null)
        {
          ...
          {
            this.eventv = EventVisitSyncer.getRunningEvent();
            if (this.eventv == null)
          ...
          this.erro = EventErrorEnum.VISIT_EVENT_UNKNOWN;
        this._client.SendPacket((SendPacket) new PROTOCOL_BASE_ATTENDANCE_ACK(this.erro, this.eventv, player._event, this.day));
      }
      catch (Exception ex)
      {
