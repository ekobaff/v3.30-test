...
// Type: PointBlank.Game.Network.ClientPacket.PROTOCOL_AUTH_SHOP_ITEM_CHANGE_DATA_REQ
...
namespace PointBlank.Game.Network.ClientPacket
{
  public class PROTOCOL_AUTH_SHOP_ITEM_CHANGE_DATA_REQ : ReceivePacket
  {
    ...
    public PROTOCOL_AUTH_SHOP_ITEM_CHANGE_DATA_REQ(GameClient Client, byte[] Buffer) => this.makeme(Client, Buffer);
    ...
    private void CreateCouponEffects(int cupomId, PointBlank.Game.Data.Model.Account p)
    {
      switch (cupomId)
      {
        ...
        case 1600010:
          ...
          if (ComDiv.updateDB("player_bonus", "fakenick", (object) p.player_name, "player_id", (object) p.player_id) && ComDiv.updateDB("players", "player_name", (object) this.Text, "player_id", (object) p.player_id))
          {
        ...
        case 1610051:
          ...
          {
            if (!ClanManager.isClanNameExist(this.Text) && ComDiv.updateDB("clans", "clan_name", (object) this.Text, "clan_id", (object) p.clanId))
            {
        ...
        case 1800047:
          ...
          {
            if (ComDiv.updateDB("players", "player_name", (object) this.Text, "player_id", (object) p.player_id))
            {
              NickHistoryManager.CreateHistory(p.player_id, p.player_name, this.Text, "Name changed (items)");
              p.player_name = this.Text;
              if (p._room != null)
              {
                using (PROTOCOL_ROOM_GET_NICKNAME_ACK roomGetNicknameAck = new PROTOCOL_ROOM_GET_NICKNAME_ACK(p._slotId, p.player_name, p.name_color))
