...
// Type: PointBlank.Game.Network.ClientPacket.PROTOCOL_LOBBY_QUICKJOIN_ROOM_REQ
...
namespace PointBlank.Game.Network.ClientPacket
{
  public class PROTOCOL_LOBBY_QUICKJOIN_ROOM_REQ : ReceivePacket
  {
    ...
    public PROTOCOL_LOBBY_QUICKJOIN_ROOM_REQ(GameClient client, byte[] data) => this.makeme(client, data);

    public override void read()
    {
      this.Select = (int) this.readC();
      Logger.warning(this.Select.ToString());
    ...
    public override void run()
    {
      try
      {
        ...
        if (this.Rooms.Count == 0)
          this._client.SendPacket((SendPacket) new PROTOCOL_LOBBY_QUICKJOIN_ROOM_ACK(2147483648U, this.Quicks));
        else
    ...
    private void getRandomRoom(Account player)
    {
      if (player != null)
      {
        ...
        if (room != null && room.getLeader(out p) && room.addPlayer(player) >= 0)
        {
          ...
          this._client.SendPacket((SendPacket) new PROTOCOL_LOBBY_QUICKJOIN_ROOM_ACK(2147483648U));
      }
      else
        this._client.SendPacket((SendPacket) new PROTOCOL_LOBBY_QUICKJOIN_ROOM_ACK(2147483648U));
    }
