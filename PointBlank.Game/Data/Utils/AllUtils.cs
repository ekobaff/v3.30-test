...
// Type: PointBlank.Game.Data.Utils.AllUtils
...
namespace PointBlank.Game.Data.Utils
{
    public static class AllUtils
    {
        ...
        public static void UpdateRanked(bool WonTheMatch, PointBlank.Game.Data.Model.Account p, DBQuery query)
        {
            //if (winnerTeam == 2)
            //    query.AddQuery("draws", (object)++p.Daily.Draws);
            //else 
            if (WonTheMatch)
                query.AddQuery("wins", (object)++p.Ranked.Wins);
            else
                query.AddQuery("loses", (object)++p.Ranked.Loses);

            query.AddQuery("matches", (object)++p.Ranked.Matches);
        }
        ...
        public static void resetBattleInfo(PointBlank.Game.Data.Model.Room room)
        {
            ...
            room.SlotRewards = null;
            room.ItensRewards = null;
        ...
        public static void EndBattle(PointBlank.Game.Data.Model.Room room) => AllUtils.EndBattle(room, room.isBotMode());
        ...
        {
            int int64 = Convert.ToInt32(DateTime.Now.ToString("yyMMddHHmm"));
            bool flag1 = false;
            bool flag2 = false;
                    ...
                    else if (itemsModel._count <= int64 & itemsModel._equip == 2)
                    {
                        if (itemsModel._category == 3 && ComDiv.getIdStatics(itemsModel._id, 1) == 16)
                        {
                            if (bonus != null)
                            {
                                ...
                                {
                                    ComDiv.updateDB("players", "name_color", (object)0, "player_id", (object)p.player_id);
                                    p.name_color = 0;
                                    if (p._room != null)
                                    {
            ...
            if (PlayerManager.CheckEquipedItems(p._equip, p._inventory._items) <= 0)
                return;
            p.SendPacket((SendPacket)new PROTOCOL_SERVER_MESSAGE_CHANGE_INVENTORY_ACK(p));
            slot._equip = p._equip;
        }

        public static void TryBalancePlayer(PointBlank.Game.Data.Model.Room room, PointBlank.Game.Data.Model.Account player, bool inBattle, ref Slot mySlot)
        {
        ...
        public static void GetXmasReward(PointBlank.Game.Data.Model.Account p)
        {
            ...
            p.SendPacket((SendPacket)new PROTOCOL_INVENTORY_GET_INFO_ACK(0, p, new ItemsModel(702001024, 1, "Xmas Event", 1, 100L)));
        }
        ...
        public static void BattleEndKills(PointBlank.Game.Data.Model.Room room) => AllUtils.BaseEndByKills(room, room.isBotMode());

        public static void BattleEndKills(PointBlank.Game.Data.Model.Room room, bool isBotMode) => AllUtils.BaseEndByKills(room, isBotMode);
        ...
        public static void BattleEndKillsFreeForAll(PointBlank.Game.Data.Model.Room room) => AllUtils.BaseEndByKillsFreeForAll(room);
        ...
        public static bool Have2ClansToClanMatch(PointBlank.Game.Data.Model.Room room) => AllUtils.GetClanListMatchPlayers(room).Count == 2;
